$(
    $('input[name="item-check"]').on('click', function () {
        checkItem(this);
    }),
    $('input[name="check-all"]').on('click', function () {
        ...
        let item = $('input[name="item-check"]')
        item.each(function () {
            checkItem(this);
        })
    }),
    $('button[name="count"]').on('click', function () {
        littleTotal(this);
    }),
    $('.shop-check').on('click', function () {
        shop_name = $(this).attr('name').split('-')[1];
        let item = $('.item-check-' + shop_name);
    ...
    $('.settlement').on('click', function () {
        ...
        var r = confirm("总价格为" + total_price);
        if (r == true) {
...
function checkItem(item) {
    let item_check = $(item).prop('checked');
    let total_price = parseFloat($('.total-price').text().slice(1));
    let line = $(item).parent().parent();
    let id = line.attr('id').split('-')[1];
    let line_price = $('.item-price-' + id);
    let count = parseInt(line.find('p').text());
    let price = parseFloat(line_price.text().slice(1)) * count;
    let shop = line.find('input').attr('class').split('-')[2];
    if (item_check) {
        line.css('background', '#fff4e8');
        total_price += price;
        let shop_class = $('.' + line.find('input').attr('class'));
        shop_class.each(function () {
            if ($(this).prop('checked')) {
                $('input[name="shop-' + shop + '"]').prop('checked', true)
            } else {
                $('input[name="shop-' + shop + '"]').prop('checked', false)
                return false;
            }
        })
    } else {
...
function littleTotal(item) {
    var chocie_count = $(item).prev();
    var count = 0
    if (chocie_count.length) {
        count = parseInt(chocie_count.text()) + 1;
        chocie_count.text(count);
    } else {
        chocie_count = $(item).next();
        count = parseInt(chocie_count.text()) - 1;
        if (count == 0) {
            confirm("购买数量最小为1")
            count = 1;
    ...
    little_price = $(item).parent().parent().prev().children().text().slice(1);
    total = parseFloat(little_price) * count;
    $(item).parent().parent().next().children().text('￥' + total.toFixed(2))
}
...
function deleteChoice() {
    ...
    all.each(function () {
        if ($(this).prop('checked')) {
            let delete_choice = $(this).parent().parent()
            delete_choice.prev().remove();
            delete_choice.remove();
        }
    })
    let shop_all = $('.shop-check')
    shop_all.each(function () {
        if ($(this).prop('checked')) {
...
var prov_select = $('#add_p');
var city_select = $('#add_c');
var country_select = $('#add_a');
provice.forEach(function(val, index) {
    prov_select.append('<option value="'+index+'">'+val.name+'</option>')
});

$('#add_p').on('change',function(){
    var index = this.value;
    city_select.empty();
    provice[index].city.forEach(function (val, index) {
        city_select.append('<option value="'+index+'">'+val.name+'</option>');
    })
})
$('#add_c').blur(function(){
    ...
    provice[index].city[new_index].districtAndCounty.forEach(function (val, index) {
        country_select.append('<option value="'+index+'">'+val+'</option>')
    });
})
